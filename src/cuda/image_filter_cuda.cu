    ...
    bool isHorizontal)
{
    ...
    extern __shared__ uchar3 shared_mem[];
    ...
    if(threadIdx.x < radius){
        ...
        shared_mem[threadIdx.y * blockDim.x + threadIdx.x] = input[input_index];
    }
    if(threadIdx.x >= blockDim.x - radius){
        ...
        shared_mem[threadIdx.y * blockDim.x + threadIdx.x + 2 * radius] = input[input_index];
    }

    // current pixel value
    shared_mem[threadIdx.y * blockDim.x + threadIdx.x + radius] = input[dispatch_index];
    __syncthreads();

    uint3 sum = make_uint3(0, 0, 0);
    for(int i = 0; i <= 2 * radius; ++i){
        uchar3 pixel = shared_mem[threadIdx.y * blockDim.x + threadIdx.x + i];
        sum.x += pixel.x;
        sum.y += pixel.y;
        sum.z += pixel.z;
    }
